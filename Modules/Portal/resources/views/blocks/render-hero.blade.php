...
<section class="relative min-h-[250px] overflow-hidden flex items-center justify-center text-center text-white"
         style="background-size: 200% 200%; background-image: linear-gradient(135deg, var(--portal-hero-from), var(--portal-hero-to), var(--portal-hero-glow), var(--portal-hero-from)); animation: heroGradient 3s ease forwards; padding: 40px 20px">
    <div class="absolute -left-32 -top-32 size-96 rounded-full opacity-[0.07] blur-3xl" style="background-color: var(--portal-hero-glow); animation: floatBlob 2s ease-in-out forwards"></div>
    <div class="absolute -bottom-32 -right-32 size-96 rounded-full opacity-[0.07] blur-3xl" style="background-color: var(--portal-hero-glow); animation: floatBlob 2s ease-in-out 0.5s forwards"></div>
    ...
    <div class="relative mx-auto max-w-4xl">
        @if (!empty($d['institution_' . $l]))
            <p class="animate-fade-in text-xs font-bold uppercase tracking-[0.3em] sm:text-sm" style="color: var(--portal-hero-glow)">
                {{ $d['institution_' . $l] }}
            </p>
        ...
        @if (!empty($d['title_' . $l]))
            <h1 class="mt-1 text-3xl font-black uppercase tracking-tight drop-shadow-xl sm:text-4xl lg:text-5xl"
                ...
                ">
                <span x-text="text"></span><span x-show="!done" class="inline-block w-[3px] animate-pulse" style="height: 0.8em; background-color: var(--portal-hero-glow); vertical-align: middle; margin-left: 2px;"></span>
            </h1>
        @endif
        <div class="mx-auto animate-fade-in" style="width: 80px; height: 4px; background: linear-gradient(90deg, var(--portal-hero-glow), var(--portal-hero-to)); margin: 12px auto; border-radius: 2px; animation-delay: 0.2s"></div>
