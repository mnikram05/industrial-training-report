...
<section class="py-8">
    <div class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div class="group transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl"
            ...
            @else
                <div class="flex h-64 items-center justify-center rounded-lg" style="background-color: color-mix(in srgb, var(--portal-text) 8%, transparent); color: color-mix(in srgb, var(--portal-text) 40%, transparent)">
            ...
            @if (!empty($d['caption_' . $l]))
                <p class="mt-4 text-center text-sm font-semibold" style="color: var(--portal-text)">{{ $d['caption_' . $l] }}</p>
            @endif
        </div>
    </div>
</section>
