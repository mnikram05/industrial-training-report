@php
    $l = app()->getLocale();
    $objektifItems = json_decode($s['objektif_items'] ?? '[]', true) ?: [];
@endphp

<x-portal-layout :menuAtas="$menuAtas" :menuBawah="$menuBawah" :pageTitle="$siteTitle" portalPage="objektif-organisasi">
            ...
            <div class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
                ...
                <div class="mb-8 flex items-center gap-3">
                    ...
                    <h2 class="text-xl font-bold" style="color: #000000">
                        {{ $s['section_title_' . $l] ?? ($l === 'ms' ? 'Objektif Organisasi' : 'Organisation Objectives') }}
                    </h2>
                </div>
                ...
                <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach ($objektifItems as $item)
                        <div class="flex flex-col items-center text-center transition-all duration-300 hover:-translate-y-[5px]"
                             style="background-color: #ffffff; border: 1px solid #f0f0f0; border-radius: 15px; padding: 30px 20px; box-shadow: 0 4px 15px rgba(0, 0, 0, 0.03)">
                            @if (!empty($item['icon']))
                                <div class="mb-4 text-4xl">
                                    {!! $item['icon'] !!}
                                </div>
                            @endif
                            <h3 class="mb-2 text-base font-bold" style="color: #000000">
                                {{ $item['title_' . $l] ?? $item['title'] ?? '' }}
                            </h3>
                            <p class="text-sm leading-relaxed" style="color: #666666">
                                {{ $item['desc_' . $l] ?? $item['desc'] ?? '' }}
                            </p>
                        </div>
                    @endforeach
                </div>
