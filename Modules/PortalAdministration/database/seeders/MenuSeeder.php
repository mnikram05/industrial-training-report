...
class MenuSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $typeAtas     = DataReference::query()->where( 'label_ms', 'Atas' )->where( 'parent_id', $menuTypeParent->id )->first();
        $typeBawah    = DataReference::query()->where( 'label_ms', 'Bawah' )->where( 'parent_id', $menuTypeParent->id )->first();
        $typeLain     = DataReference::query()->where( 'label_ms', 'Lain-lain' )->where( 'parent_id', $menuTypeParent->id )->first();
        $typeNavCepat = DataReference::query()->where( 'label_ms', 'Navigasi Cepat' )->where( 'parent_id', $menuTypeParent->id )->first();

        // Parent menus
        $atas = Menu::query()->updateOrCreate(
            ['title_ms' => 'Atas', 'parent_id' => null],
            ['title_en' => 'Top', 'type_id' => $typeAtas->id, 'status_id' => 1, 'icon' => '1', 'sort' => 1],
        );

        $bawah = Menu::query()->updateOrCreate(
            ['title_ms' => 'Bawah', 'parent_id' => null],
            ['title_en' => 'Down', 'type_id' => $typeBawah->id, 'status_id' => 1, 'icon' => '1', 'sort' => 2],
        );

        Menu::query()->updateOrCreate(
            ['title_ms' => 'Lain-lain', 'parent_id' => null],
            ['title_en' => 'Others', 'type_id' => $typeLain->id, 'status_id' => 1, 'icon' => '1', 'sort' => 3],
        );

        $navCepat = Menu::query()->updateOrCreate(
            ['title_ms' => 'Navigasi Cepat', 'parent_id' => null],
        ...
        Menu::query()->updateOrCreate(
            ['title_ms' => 'RUMUSAN', 'parent_id' => $atas->id],
        ...
        Menu::query()->updateOrCreate(
            ['title_ms' => 'LATAR BELAKANG ORGANISASI', 'parent_id' => $pengenalan->id],
        ...
        Menu::query()->updateOrCreate(
            ['title_ms' => 'OBJEKTIF ORGANISASI', 'parent_id' => $pengenalan->id],
        ...
        Menu::query()->updateOrCreate(
            ['title_ms' => 'VISI & MISI', 'parent_id' => $pengenalan->id],
        ...
        Menu::query()->updateOrCreate(
            ['title_ms' => 'CARTA ORGANISASI', 'parent_id' => $pengenalan->id],
        ...
        Menu::query()->updateOrCreate(
            ['title_ms' => 'FUNGSI BAHAGIAN', 'parent_id' => $pengenalan->id],
        ...
        Menu::query()->updateOrCreate(
            ['title_ms' => 'HALATUJU ORGANISASI', 'parent_id' => $pengenalan->id],
