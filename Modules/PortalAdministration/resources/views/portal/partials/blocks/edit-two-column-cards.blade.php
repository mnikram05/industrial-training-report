<div class="space-y-4">
    ...
    <div class="flex items-center justify-end">
        <button type="button" @click="block.data.cards.push({ icon: '', title_ms: '', title_en: '', items_ms: [], items_en: [] })"
    ...
    <template x-for="(card, ci) in block.data.cards" :key="ci">
        <div class="rounded-md border p-3 space-y-3">
            <div class="flex items-center justify-between">
                ...
                <button type="button" @click="block.data.cards.splice(ci, 1)" class="text-destructive hover:text-destructive/80"><svg class="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12"/></svg></button>
            </div>
            ...
            <div class="grid gap-3 sm:grid-cols-2">
                <x-input type="text" x-model="card.title_ms" placeholder="{{ __('modules/portal/setting.fields.title_ms') }}" />
                <x-input type="text" x-model="card.title_en" placeholder="{{ __('modules/portal/setting.fields.title_en') }}" />
            </div>
            <div class="rounded border bg-gray-50/50 p-3 space-y-2">
                <div class="flex items-center justify-between">
                    ...
                    <button type="button" @click="card.items_ms.push(''); card.items_en.push('')" class="text-xs text-foreground/70 hover:text-foreground">+ {{ __('modules/portal/setting.actions.add') }}</button>
                </div>
                <template x-for="(item, bi) in card.items_ms" :key="bi">
                    <div class="flex items-start gap-2">
                        <span class="mt-2 text-xs text-muted-foreground" x-text="bi + 1"></span>
                        <div class="grid flex-1 gap-2 sm:grid-cols-2">
                            <x-input type="text" x-model="card.items_ms[bi]" placeholder="MY" />
                            <x-input type="text" x-model="card.items_en[bi]" placeholder="EN" />
                        </div>
                        <button type="button" @click="card.items_ms.splice(bi, 1); card.items_en.splice(bi, 1)" class="mt-1.5 text-destructive"><svg class="size-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12"/></svg></button>
                    </div>
                </template>
            </div>
        </div>
