        ...
        <x-card-content flush stacked>
            <x-detail-grid>
                <x-detail-field :value="$menu->title_en ?? '—'">
                    <x-slot:labelText>{{ __('modules/reference/menu.fields.title_en') }}</x-slot:labelText>
                </x-detail-field>
                ...
                <x-detail-field :value="(string) ($menu->sort ?? 0)">
                    <x-slot:labelText>{{ __('modules/reference/menu.fields.sort') }}</x-slot:labelText>
                </x-detail-field>
                <x-detail-field :value="$menu->created_at?->format('M j, Y H:i') ?? '—'">
                    <x-slot:labelText>{{ __('modules/reference/menu.fields.created_at') }}</x-slot:labelText>
                </x-detail-field>
                <x-detail-field :value="$menu->updated_at?->format('M j, Y H:i') ?? '—'">
                    <x-slot:labelText>{{ __('modules/reference/menu.fields.updated_at') }}</x-slot:labelText>
                </x-detail-field>
            </x-detail-grid>

            <x-button-group plain end spaced>
                <a href="{{ route('reference.menus.edit', $menu) }}">
                    <x-button variant="secondary">{{ __('modules/reference/menu.edit') }}</x-button>
                </a>
            </x-button-group>
        </x-card-content>
    </x-card>
