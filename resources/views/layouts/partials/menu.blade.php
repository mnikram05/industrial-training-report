<div class="space-y-4 pb-2" x-data="{ cmsOpen: @js($cmsOpen), referencesOpen: @js($referencesOpen), portalOpen: @js($portalOpen) }">
    <x-sidebar-group>
        ...
        <x-sidebar-menu-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" :title="__('sidebar.dashboard')"
            ...
            <svg class="size-4 shrink-0 text-white/45 group-hover:text-white/90" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                stroke-width="2">
        ...
        <button type="button" @click="cmsOpen = !cmsOpen"
            x-bind:class="$store.layout.sidebarCollapsed ? 'justify-center px-0' : ''"
            class="group flex h-8 w-full items-center gap-2 rounded-md px-2 text-left text-[13px] font-medium text-white/85 transition-colors hover:bg-[color-mix(in_srgb,var(--portal-accent)_22%,transparent)] hover:text-white">
            <svg class="size-4 shrink-0 text-white/45 group-hover:text-white/85" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                stroke-width="2">
