<nav x-data="{ open: false, profileOpen: false, langOpen: false }" x-init="$store.layout.boot()" class="contents">
    <div class="fixed inset-x-0 top-0 z-30 border-b border-border bg-background/95 backdrop-blur lg:hidden">
        <div class="flex h-14 items-center justify-between px-4">
            <a href="{{ route('dashboard') }}"
                class="inline-flex items-center gap-2 text-sm font-semibold text-foreground">
                <svg class="size-5 text-foreground" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                    stroke-width="1.8">
            ...
            <button type="button" @click="open = !open"
                class="inline-flex items-center justify-center rounded-md border border-input bg-background p-2 text-muted-foreground shadow-xs transition hover:bg-accent hover:text-foreground">
    ...
    <aside
        ...
        class="js-sidebar fixed inset-y-0 left-0 z-50 flex w-60 -translate-x-full transform flex-col border-r border-border bg-background transition duration-300 ease-out lg:translate-x-0">
        <x-sidebar-header class="!border-b-0 border-border bg-background px-2.5 py-2.5">
            <div class="relative" @click.outside="langOpen = false">
                <button type="button" @click="langOpen = !langOpen"
                    ...
                    class="inline-flex h-11 w-full items-center gap-2.5 rounded-lg border border-input bg-background px-2.5 text-left shadow-xs transition-colors hover:bg-accent/40">
                    ...
                    <span class="min-w-0 flex-1" x-show="!$store.layout.sidebarCollapsed">
                        <span
                            class="block truncate text-sm font-semibold text-foreground">{{ config('app.name') }}</span>
                        <span class="block truncate text-[11px] text-muted-foreground">{{ $localeLabel }}</span>
                    </span>
        ...
        <x-sidebar-content class="bg-background px-1.5 py-1.5">
            @include('layouts.partials.menu')
        </x-sidebar-content>

        <x-sidebar-footer class="!border-t-0 border-border bg-background p-2">
            <div class="relative" @click.outside="profileOpen = false">
                ...
                <button type="button" @click="profileOpen = !profileOpen"
                    :class="$store.layout.sidebarCollapsed ? 'justify-center px-0' : ''"
                    class="inline-flex w-full items-center gap-3 rounded-md px-2 py-2 text-left transition-colors hover:bg-accent/60">
                    <x-avatar>
                        <x-avatar-fallback>{{ $avatarFallback }}</x-avatar-fallback>
                        <x-avatar-badge :impersonating="$isImpersonating" />
                    </x-avatar>
